 ...
 * @date : 2019/8/2
 * @time : 15:48
 * @Description : jpa repository注册配置类（快速修改源代码方式：工程中有相同类的时候优先使用源码的类而不是jar包的类）
 */
public class RepositoryConfigurationDelegate {
    ...
    public List<BeanComponentDefinition> registerRepositoriesIn(BeanDefinitionRegistry registry, RepositoryConfigurationExtension extension) {
        ...
        while(var5.hasNext()) {
            ...
            // 判断配置类是否使用primary进行了标注，若有，就设为primary
            if (metadata.hasAnnotation(Primary.class.getName())) {  // 判断是否含primary注解
                beanDefinition.setPrimary(true);
            } else if (metadata.hasAnnotation(RepositoryBeanNamePrefix.class.getName())) {
                // 再判断是否使用了RepositoryBeanNamePreFix进行标注，若有，添加名称前缀
